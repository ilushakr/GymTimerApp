...
internal class WorkoutViewModel(
    ...
    private var currentWorkout: GymTimer.WorkoutModel? = null
        ...
        .map { currentExerciseState ->
            when (currentExerciseState) {
                is GymTimer.CurrentExerciseState.CurrentExercise -> State.WorkoutUiModel(
                    ...
                    numberOfSets = currentWorkout
                        ?.exerciseList
                        ?.firstOrNull { it.name == currentExerciseState.name }
                        ?.numberOfSets
                        ?: 0,
    ...
    val exerciseListState = gymTimer
        .currentExerciseModelStateFlow
        .filterIsInstance<GymTimer.CurrentExerciseState.CurrentExercise>()
        .distinctUntilChanged { old, new -> old.name == new.name }
        .map { currentExercise ->
            currentWorkout?.exerciseList?.let { exerciseList ->
                ExerciseListUiModel(
                    exerciseNameList = exerciseList.map { it.name },
                    currentExerciseIndex = exerciseList.indexOfFirst { it.name == currentExercise.name }
                )
            }
        }
...
@Stable
internal data class ExerciseListUiModel(
    val exerciseNameList: List<String>,
    val currentExerciseIndex: Int
)


private val wednesdayWorkout = GymTimer.WorkoutModel(
    listOf(
        GymTimer.ExerciseModel(
            name = "Тяга сверху",
            numberOfSets = 6,
            workDuration = 40.seconds,
            restDuration = 1.minutes,
            finishWorkRemainingDuration = 10.seconds,
            finishRestRemainingDuration = 15.seconds,
        ),
        GymTimer.ExerciseModel(
            name = "Тяга в кроссовере",
            numberOfSets = 4,
            workDuration = 40.seconds,
            restDuration = 1.minutes,
            finishWorkRemainingDuration = 10.seconds,
            finishRestRemainingDuration = 20.seconds,
        ),
        GymTimer.ExerciseModel(
...
private val fridayWorkout = GymTimer.WorkoutModel(
    listOf(
        GymTimer.ExerciseModel(
            name = "Брусья",
            numberOfSets = 6,
